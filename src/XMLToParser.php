...
$xml = XMLToParser::encode( $data , 'response' );
echo $xml->asXML();
...
class XMLToParser
{
    ...
    private static $_defaultStandalone = 'no';
    ...
    public static function encode($data, $root = null)
    {
        ...
        try {
            ...
            $standalone = self::$_defaultStandalone;
            $xml_string = "<?xml version=\"{$version}\" encoding=\"{$encoding}\" standalone=\"{$standalone}\" ?>";
    ...
    private static function _addChildren(SimpleXMLElement $element, $data)
    {
        foreach ($data as $key => $value) {
            ...
            $node = self::_formatName(is_numeric($key) ? (is_array($value) ? self::$_defaultListNode : self::$_defaultItemNode) : $key);
            if (is_array($value)) {
